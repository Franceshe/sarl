...
public class EcjBatchCompiler implements IJavaBatchCompiler {
	...
	public boolean compile(File classDirectory, Iterable<File> sourcePathDirectories,
		...
		if (!bootClassPathEntries.isEmpty() && !Strings.isEmpty(sourceJavaVersion)) {
			final JavaVersion jversion = JavaVersion.fromQualifier(sourceJavaVersion);
		...
		commandLineArguments.add("-" + sourceJavaVersion); //$NON-NLS-1$
		if (!Strings.isEmpty(targetJavaVersion)) {
			commandLineArguments.add("-target " + targetJavaVersion); //$NON-NLS-1$
		}
